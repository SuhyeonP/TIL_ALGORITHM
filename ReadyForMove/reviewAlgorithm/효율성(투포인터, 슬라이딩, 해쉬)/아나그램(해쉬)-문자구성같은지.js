...
// ▣ 입력설명
// 첫 줄에 첫 번째 단어가 입력되고,
// 두 번째 줄에 두 번째 단어가 입력됩니다. 단어의 길이는 100을 넘지 않습니다.
...
function solution(str1, str2) {
    ...
    for (let ele of str1) {
        bucket.set(ele, bucket.has(ele) ? bucket.get(ele) + 1 : 1);
    }

    for(let ele of str2) {
        // 0 인경우 str2는 아직 가진 문자열인데 빼줘서 개수가 안맞는거임

        if(!bucket.has(ele) || bucket.get(ele) === 0) return 'NO';
        else {
            bucket.set(ele, bucket.get(ele) - 1);
        }
    }
