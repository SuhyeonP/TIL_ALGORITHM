...
// 시간은 첫날 0시를 0으로 해서
// 마지막날 밤 12시를 72로 하는 타임라인으로 오는 시간과 가는 시간이
// 음이 아닌 정수로 표현됩니다.
...
// [18, end]
// 이런식으로 만들고 정렬해야함
// 정렬은 숫자 오름차순인데 같은수인데 start, end가 다르면 end가 먼저 와야함
// end 시간은 없기때문
...
function solution(times) {
    ...
    for(const [time, type] of timeArr) {
        if (type === 0) {
            count += 1;
        } else {
            count -= 1;
...
function solution2(times) {
    ...
    for (const [start, end] of times) {
        timeTable.push([start, 0]);
        timeTable.push([end, 1]);
    }

    timeTable.sort((a, b) => {
        if (a[0] === b[0]) {
            // 끝난 타임이 먼저 와야 떠난 사람 뺄 수 있음
            return b[1] - a[1];
        } else {
            return a[0] - b[0];
        }
    ...
    let count = 0;
    let answer = 0;
    for (const [time, type] of timeTable) {
        if(type) {
            count -= 1;
        } else {
            count += 1;
